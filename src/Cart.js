...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    try {
      ...
      console.log('Token:', token); // Debugging line to verify token
  
      // Make the request to create the order
      const response = await axios.post(
        'http://localhost:5000/create-order',
        { amount: totalAmount }, // Pass the amount as data
        {
          headers: {
            'Authorization': `Bearer ${token}`, // Add the token to the Authorization header
          }
      ...
      console.log('Order Creation Response:', response.data); // Debugging line to check the response
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            console.log('Payment Response:', response); // Debugging line to check the payment response
            ...
            fetchCartItems(); // Refresh cart items after successful payment
          } catch (err) {
      ...
      <footer>
        <div class="footer-content">
          <div class="footer-logo"> 
            <img src={logo} alt="Your Brand" style={{ width: '120px', height: 'auto' }} />
          </div>
          <ul class="footer-nav">
